...
let advance (nfa, w, ((i, j), p)) =
  ...
  let tr = List.fold_left (
    fun tr (u, v, k) -> ttr_add tr u v (IntSet.singleton k) IntSet.empty IntSet.empty
  ) TTNull (Nfa.get_transitions nfa i) in
  (* combine with all the transitions of the second NFA *)
  let tr = List.fold_left (
    fun tr (u, v, k) -> ttr_add tr u v IntSet.empty (IntSet.singleton k) IntSet.empty
  ) tr (Nfa.get_transitions nfa j) in
  (* finally add all the transitions of the phi *)
  let tr = IntSet.fold (
    fun i tr -> List.fold_left (
      fun tr (u, v, k) -> ttr_add tr u v IntSet.empty IntSet.empty (IntSet.singleton k)
