...
rule tk_normal = 
  parse '\n' { Eos }
    ...
    |normal_literal as u { Literal(get_pos lexbuf, resolve_literal u (get_spos lexbuf)) }
    ...
    |'\\' (_ as c) { Literal(get_pos lexbuf, resolve_char c (get_spos lexbuf)) } 
    |eof { raise ParsingData.UnexpectedEndOfInput }
and tk_class_header =
  parse '\n' { Eos }
    |range_chead { ClsRange(resolve_literal u (get_spos lexbuf), resolve_literal v (get_spos lexbuf)) }
    |chead_literal as u { Literal(get_pos lexbuf, resolve_literal u (get_spos lexbuf)) }
    |predefined_cls as cls { ClsNamed(get_pos lexbuf, resolve_class cls) }
    |'\\'(['1' - '9' 'a' - 'z' 'A' - 'Z' '\n'] as c) { raise (ParsingData.UnsupportedEscape(get_epos lexbuf, c)) }
    |'\\' (_ as c) { Literal(get_pos lexbuf, resolve_char c (get_spos lexbuf)) } 
    |eof { raise ParsingData.UnexpectedEndOfInput }
and tk_class_body =
  parse '\n' { Eos }
    |range_cbody { ClsRange(resolve_literal u (get_spos lexbuf), resolve_literal v (get_spos lexbuf)) }
    |cbody_literal as u { Literal(get_pos lexbuf, resolve_literal u (get_spos lexbuf)) }
    |predefined_cls as cls { ClsNamed(get_pos lexbuf, resolve_class cls) }
    |'\\'(['1' - '9' 'a' - 'z' 'A' - 'Z' '\n'] as c) { raise (ParsingData.UnsupportedEscape(get_epos lexbuf, c)) }
    |'\\' (_ as c) { Literal(get_pos lexbuf, resolve_char c (get_spos lexbuf)) } 
...
and tk_quote =
  parse '\n' { Eos }
    ...
    |_ as u { Literal(get_pos lexbuf, resolve_char u (get_spos lexbuf)) }
    |eof { raise ParsingData.UnexpectedEndOfInput }
